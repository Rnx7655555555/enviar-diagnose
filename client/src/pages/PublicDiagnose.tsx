...
import { capabilityLabel } from "@/scanner/capability";
import { assertFileAccepted, detectArchiveFormat } from "@/scanner/security";
import type { ArchiveFormat, Evidence, ExternalPanelFinding, ExternalPanelReport, ExternalPanelResult, JailbreakFinding, JailbreakReport, JailbreakResult, ManualReview, ScanReport, ScanResult, SignatureDefinition, WorkerEvent, WorkerProgress } from "@/scanner/types";
...
const externalPanelMeta: Record<ExternalPanelResult, ResultMeta> = {
  yes: { label: "EXTERNAL: SIM", description: () => "Uma ferramenta externa foi identificada por identificador completo em fonte técnica estruturada. Isso não atribui uso de cheat ao Free Fire.", tone: "border-cyan-300/25 bg-cyan-300/[.07]", badge: "bg-cyan-300/15 text-cyan-100 ring-1 ring-cyan-300/25" },
  no: { label: "EXTERNAL: NÃO", description: () => "Nenhuma evidência estruturada suficiente foi observada nas fontes passivas disponíveis. Isso não prova ausência de painel.", tone: "border-emerald-300/25 bg-emerald-300/[.07]", badge: "bg-emerald-300/15 text-emerald-100 ring-1 ring-emerald-300/25" },
  manual: { label: "EXTERNAL: VERIFICAR", description: (_exact, review) => `${review} sinal${review === 1 ? "" : "is"} técnico${review === 1 ? "" : "s"} encontrado${review === 1 ? "" : "s"}; sem confirmação automática.`, tone: "border-amber-300/30 bg-amber-300/[.07]", badge: "bg-amber-300/15 text-amber-100 ring-1 ring-amber-300/30" },
};
...
function Progress({ value, onCancel }: { value: ProgressState; onCancel: () => void }) {
  const discovered = value.discoveredFileCount ? ` de ${value.discoveredFileCount} identificados` : "";
  return <section className="mt-8 overflow-hidden rounded-[1.4rem] border border-cyan-300/15 bg-[#10161e]"><div className="p-5 sm:p-6"><div className="flex items-start justify-between gap-5"><div><p className="text-[10px] font-semibold tracking-[.2em] text-cyan-200/80">PROCESSAMENTO LOCAL · ETAPA {value.step}/5</p><h2 className="mt-2 text-lg font-semibold text-slate-100">{value.stage}</h2>{value.currentFile && <p className="mt-2 max-w-md truncate font-mono text-[11px] text-slate-500">Arquivo: {value.currentFile.split("/").at(-1)}</p>}</div><span className="font-mono text-lg text-cyan-200">{typeof value.progress === "number" ? `${value.progress}%` : "…"}</span></div><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full bg-cyan-300 transition-[width] duration-300" style={{ width: `${Math.max(2, value.progress ?? 2)}%` }} /></div><div className="mt-4 grid gap-2 text-xs text-slate-500 sm:grid-cols-2"><span><Loader2 className="mr-1.5 inline h-3.5 w-3.5 animate-spin text-cyan-200" />{value.bytesRead ? `${formatBytes(value.bytesRead)} processados localmente` : "Preparando os recursos locais"}</span><span className="sm:text-right">Arquivos: {value.analyzedFileCount ?? value.processedFileCount}{discovered}</span><span>Tempo: {formatDuration(Date.now() - value.startedAt)}</span><span className="sm:text-right">{value.relevantFileCount} fonte{value.relevantFileCount === 1 ? "" : "s"} técnica{value.relevantFileCount === 1 ? "" : "s"}</span></div></div><div className="flex justify-end border-t border-white/[.07] px-5 py-3 sm:px-6"><button onClick={onCancel} className="inline-flex items-center gap-2 text-xs text-slate-400 transition-colors hover:text-slate-100"><X className="h-3.5 w-3.5" />Cancelar análise</button></div></section>;
}
...
function ExternalPanelFindingCard({ item }: { item: ExternalPanelFinding }) {
  const confirmed = item.assessment === "confirmada";
  const color = confirmed ? "cyan" : "amber";
  return <article className={`rounded-[1.25rem] border bg-[#10161e] p-5 shadow-[0_14px_45px_rgba(0,0,0,.18)] ${confirmed ? "border-cyan-300/20" : "border-amber-300/25"}`}><div className={`border-l-2 pl-4 ${confirmed ? "border-cyan-300" : "border-amber-300"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-semibold tracking-wide text-slate-100">{item.label}</h3><p className="mt-1 text-xs text-slate-500">{confirmed ? "Identificador completo confirmado em fonte técnica estruturada." : "Triagem passiva; precisa de conferência antes de qualquer conclusão."}</p></div><span className={`rounded-md border px-2 py-1 text-[10px] font-bold tracking-[.14em] ${confirmed ? "border-cyan-300/30 bg-cyan-300/[.08] text-cyan-100" : "border-amber-300/30 bg-amber-300/[.08] text-amber-100"}`}>{confirmed ? "SIM" : "VERIFICAR"}</span></div><Collapsible><CollapsibleTrigger asChild><button className={`mt-5 flex w-full items-center justify-center gap-2 rounded-lg border px-4 py-3 text-[11px] font-semibold tracking-[.12em] transition-colors ${color === "cyan" ? "border-cyan-300/25 bg-cyan-300/[.06] text-cyan-100 hover:bg-cyan-300/[.12]" : "border-amber-300/25 bg-amber-300/[.06] text-amber-100 hover:bg-amber-300/[.12]"}`}><Search className="h-3.5 w-3.5" />VER LOG DO RESULTADO</button></CollapsibleTrigger><CollapsibleContent><div className="mt-4 grid gap-3"><Info label="INDICADOR" value={item.indicator} mono /><div className="grid gap-3 sm:grid-cols-2"><Info label="FAMÍLIA" value={item.family} /><Info label="TIPO DE CORRESPONDÊNCIA" value={item.matchType} /></div><Info label="REGRA" value={item.rule} /><Info label="ARQUIVO" value={item.sourcePath} mono />{item.context && <Info label="CONTEXTO" value={item.context} mono />}</div></CollapsibleContent></Collapsible></div></article>;
}
...
function ExternalPanelSummary({ externalPanel }: { externalPanel: ExternalPanelReport }) {
  ...
  return <section className={`mt-5 rounded-[1.5rem] border p-6 text-center sm:p-8 ${meta.tone}`}><p className="text-[10px] font-semibold tracking-[.22em] text-slate-400">TRIAGEM DE EXTERNAL / ESIGN / ASSINATURAS</p><div className="mt-5 flex justify-center"><span className={`rounded-full px-4 py-2 text-xs font-bold tracking-[.14em] ${meta.badge}`}>{meta.label}</span></div><p className="mx-auto mt-5 max-w-md text-sm leading-6 text-slate-300">{externalPanel.summary}</p><div className="mx-auto mt-4 max-w-md rounded-xl border border-white/[.08] bg-black/15 px-4 py-3 text-left"><p className="text-[10px] font-semibold tracking-[.14em] text-cyan-200/75">COBERTURA PASSIVA · {externalPanel.coverage.status === "available" ? "BASE DISPONÍVEL" : "LIMITADA"}</p><p className="mt-2 text-xs leading-5 text-slate-400">{externalPanel.coverage.note}</p></div><p className="mt-3 text-xs text-slate-500">{externalPanel.sourcesReviewed} fonte{externalPanel.sourcesReviewed === 1 ? "" : "s"} técnica{externalPanel.sourcesReviewed === 1 ? "" : "s"} avaliada{externalPanel.sourcesReviewed === 1 ? "" : "s"} localmente.</p>{externalPanel.findings.length > 0 && <Collapsible><CollapsibleTrigger asChild><button className="mt-5 inline-flex items-center gap-2 text-[11px] font-semibold tracking-[.12em] text-slate-200 hover:text-amber-100"><ChevronDown className="h-3.5 w-3.5" />SINAIS TÉCNICOS</button></CollapsibleTrigger><CollapsibleContent><div className="mt-5 space-y-3 text-left">{externalPanel.findings.map(item => <ExternalPanelFindingCard key={`${item.id}-${item.sourcePath}`} item={item} />)}</div></CollapsibleContent></Collapsible>}</section>;
}

function TechnicalCoverage({ report }: { report: ScanReport }) {
  const { coverage, capacity } = report;
  const items = [
    ["ARQUIVO", report.fileName], ["TAMANHO", formatBytes(report.fileSize)], ["FORMATO", report.fileFormat.toUpperCase()], ["TEMPO", formatDuration(report.durationMs)],
    ["ARQUIVOS ENCONTRADOS", String(coverage.totalFiles)], ["FONTES ANALISADAS", String(coverage.analyzedFiles)], ["ARQUIVOS IGNORADOS", String(coverage.ignoredFiles)], ["FONTES LIMITADAS", String(coverage.limitedFiles)],
    ["PLISTS", String(coverage.plistFiles)], ["PERFIS", String(coverage.profileFiles)], ["CERTIFICADOS", String(coverage.certificateFiles)], ["IDENTIFICADORES", String(coverage.identifiersExtracted)],
    ["REGRAS EXECUTADAS", String(coverage.rulesExecuted)], ["EVIDÊNCIAS", String(coverage.evidenceCount)], ["PROCESSAMENTO", `CAPACIDADE ${capabilityLabel(capacity.level)}`], ["CHUNK", formatBytes(capacity.chunkBytes)],
  ];
  return <section className="mt-5 rounded-[1.5rem] border border-white/[.08] bg-[#10161e] p-5 sm:p-6"><p className="text-[10px] font-semibold tracking-[.2em] text-cyan-200/80">DIAGNÓSTICO TÉCNICO DO SCAN</p><p className="mt-2 text-xs leading-5 text-slate-500">Resumo de processamento local; não armazena o conteúdo bruto do arquivo.</p><div className="mt-5 grid gap-2 sm:grid-cols-2">{items.map(([label, value]) => <Info key={label} label={label} value={value} mono={label === "ARQUIVO" || label === "CHUNK"} />)}</div>{capacity.fallback && <p className="mt-4 text-xs leading-5 text-slate-500">O navegador não informou memória disponível; o scanner usou o perfil de segurança compatível.</p>}</section>;
}
...
export default function PublicDiagnose() {
  ...
  const [format, setFormat] = useState<ArchiveFormat | null>(null);
  ...
  const selectFile = async (candidate?: File) => { if (!candidate) return; try { assertFileAccepted(candidate); setFormat(await detectArchiveFormat(candidate)); setFile(candidate); setReport(null); } catch (error) { toast.error(error instanceof Error ? error.message : "Arquivo inválido."); } };
