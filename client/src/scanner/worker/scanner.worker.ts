...
import { correlateEvidence, detectInContent, scoreReport } from "../detector";
import { parseFileValues } from "../parsers";
import { buildTree, detectArchiveFormat, isRelevantSysdiagnosePath, safeArchivePath, scanLimits } from "../security";
import type { Evidence, ScanReport, SignatureDefinition, WorkerEvent } from "../types";
...
class TarWalker {
  ...
  private expanded = 0;
  constructor(private readonly paths: Array<{ path: string; size: number }>, private readonly onFile: FileVisitor) {}
  ...
  async push(input: Uint8Array) {
    ...
    while (!cancelled) {
      ...
      if (entry.remaining > 0) {
        ...
        if (isRelevantSysdiagnosePath(entry.path) && entry.captureSize < scanLimits.maxRelevantFileBytes) {
      ...
      this.current = null;
      if (isRelevantSysdiagnosePath(entry.path)) await this.onFile(entry.path, entry.capture.length ? entry.capture.reduce(concat, new Uint8Array()) : new Uint8Array());
    }
  }
}

async function inspectTarGz(file: File, visit: FileVisitor, update: (read: number) => void) {
  const paths: Array<{ path: string; size: number }> = [];
  const walker = new TarWalker(paths, visit);
...
async function inspectZip(file: File, visit: FileVisitor, update: (read: number) => void) {
  const paths: Array<{ path: string; size: number }> = [];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (!entry.directory && isRelevantSysdiagnosePath(path)) {
...
function recommendations(evidence: Evidence[]) {
  if (!evidence.length) return ["Nenhuma evidência contextual foi identificada pelas regras ativas.", "Mantenha o arquivo original se for necessária uma análise independente."];
  return ["Revise os caminhos, chaves e trechos exibidos antes de qualquer conclusão.", "O resultado é uma triagem técnica baseada em regras e não substitui análise forense ou diagnóstico definitivo."];
}
...
async function scan(file: File, signatures: SignatureDefinition[]) {
  ...
  const visit: FileVisitor = async (path, data) => {
    ...
    evidence.push(...detectInContent(path, parsed.text, parsed.values, signatures));
  };
  ...
  const paths = format === "tar.gz" ? await inspectTarGz(file, visit, update) : await inspectZip(file, visit, update);
  if (cancelled) return;
  ...
  const outcome = scoreReport(evidence);
  const report: ScanReport = { id: crypto.randomUUID(), fileName: file.name, fileSize: file.size, fileFormat: format, durationMs: Date.now() - startedAt, createdAt: Date.now(), ...outcome, processedFileCount: paths.filter(item => item.path && !item.path.endsWith("/")).length, relevantFileCount: relevant, evidence, recommendations: recommendations(evidence), limitations: Array.from(new Set(limitations)), tree: buildTree(paths) };
